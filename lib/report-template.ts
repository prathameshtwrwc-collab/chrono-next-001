...
function stableId(...parts: (string | number | null | undefined)[]): string {
  ...
  for (let i = 0; i < s.length; i++) {
    ...
    hash = ((hash << 7) - hash + c) | 0;
  }
  return "RPT-" + Math.abs(hash).toString(16).toUpperCase().slice(0, 8);
}
...
export function buildReportHtml(data: ReportData) {
  const ct = data.chronotype || "EAGLE";
  ...
  const userRecs = data.recommendations?.length ? data.recommendations : defaultRecs[ct] || [];
